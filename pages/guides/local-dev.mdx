...
## Prerequisites
- a [generated microservice](/cli/generate) in your [wave](/getting-started/setting-up-your-first-monorepo) monorepo.
...
This will start your local GraphQL Server via Yoga as well as load your `schema.prisma` file into a local SQLite database.

### Configure your ApolloClient
Replace your Appsync API url with your local GraphQL Server url inside `apollo.tsx`
```js {7} filename="/apollo.tsx"
...
This will generate a file called `graphql-types.ts` inside the `apps/client/graphql` directory. Your hooks are at the bottom of this file, equipped with usage information.
...
## Using Hooks
After running `npx wave codegen`, you'll have a generated file called `graphql-types.ts`. At the bottom of this file, you'll find your generated hooks. Here's some examples of how to use them.
...
import { Button, Box } from "@vastly/ui";
...
export default function App() {
  ...
  return (
    <Box>
      ...
      <Button onClick={async() => await getUsers().then(({data}) => console.log("users: ", data))}>
        List User
      </Button>
    </Box>
  )};
  ...
  The above example shows how you can use a mutation or a lazy query with a trigger function. Each hook in this example has a destructured array of variables. The first variable is a function that triggers the hook and the second <i>optional</i> argument is the data that is returned from the event.