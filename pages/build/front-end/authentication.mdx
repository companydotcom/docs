...
<Callout>Ensure you have the [Vasly CLI](/cli) installed on your machine.</Callout>
...
Run the `vastly generate ciam` command from the root of your monorepo.
...
At this step, Wave automatically generates some boilerplate code for you to easily modify or configure as needed.
...
- An `api` folder will be created under your `pages` directory, with a new `auth/[...nextauth].js` file.

This file sets up a small api, that lives directly in your app, and wires up that API with your selected auth provider (in this case, AWS Cognito).

<Callout type="warning" emoji="⚠️">
    To get these variables, you need to have an AWS Cognito UserPool setup on your AWS account already.
</Callout>
...
For more information on what is possible with NextAuth, check out their user docs [here](https://next-auth.js.org/providers/).
