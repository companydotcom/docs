...
* #### Sync secrets from DynamoDB to GitHub
    This workflow is designed to dynamically synchronize secrets from an AWS DynamoDB table to GitHub based on inputs passed from the `base-configuration` workflow. By using a single source of truth, we won't have to worry about missing secrets or variables.
...
1. **Setup Reusable Workflows:**
   - Ensure the reusable workflows (`sync-secrets.yml` and `deploy-app.yml`) are correctly defined and located in the `shared-workflows/.github/workflows/` directory.

2. **Define Base Configuration Workflow:**
   - Add the `base-configuration.yml` file to your repository's `.github/workflows/` directory. This workflow will call the reusable workflows.

3. **Create Specific Application Workflows:**
